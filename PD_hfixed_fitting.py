...
def rmse(pred, expt):
  return np.sqrt(np.mean((pred-expt)**2))
...
for i in range(h_array.size):
  ...
  err = rmse(pd_subset, exptdata['% dissolved'])
  errorray[i] = err  
  if err < min_err:
    min_err = err
    h_opt = h
...
print("\n****\nOptimal h: ", h_opt, "RMSE: %.2f"%min_err)
time, PD = predict_PD(h_opt)
#plt.plot(time, PD, c='C1', label='Distribution')
